...
#include "LE_SpringArmComponent.h"
...
//////////////////////////////////////////////////////////////////////////
// ULE_SpringArmComponent

const FName ULE_SpringArmComponent::SocketName(TEXT("SpringEndpoint"));

ULE_SpringArmComponent::ULE_SpringArmComponent(const FObjectInitializer& ObjectInitializer)
...
FRotator ULE_SpringArmComponent::GetDesiredRotation() const {
	return GetComponentRotation();
}

FRotator ULE_SpringArmComponent::GetTargetRotation() const {
...
void ULE_SpringArmComponent::UpdateDesiredArmLocation(bool bDoTrace, bool bDoLocationLag, bool bDoRotationLag, float DeltaTime) {
...
FVector ULE_SpringArmComponent::BlendLocations(const FVector& DesiredArmLocation, const FVector& TraceHitLocation, bool bHitSomething, float DeltaTime) {
	return bHitSomething ? TraceHitLocation : DesiredArmLocation;
}

void ULE_SpringArmComponent::ApplyWorldOffset(const FVector & InOffset, bool bWorldShift) {
	Super::ApplyWorldOffset(InOffset, bWorldShift);
	PreviousDesiredLoc += InOffset;
	PreviousArmOrigin += InOffset;
}

void ULE_SpringArmComponent::OnRegister() {
	Super::OnRegister();
...
void ULE_SpringArmComponent::PostLoad() {
	Super::PostLoad();
}

void ULE_SpringArmComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction) {
	Super::TickComponent(DeltaTime, TickType, ThisTickFunction);
	UpdateDesiredArmLocation(bDoCollisionTest, bEnableCameraLag, bEnableCameraRotationLag, DeltaTime);
}

FTransform ULE_SpringArmComponent::GetSocketTransform(FName InSocketName, ERelativeTransformSpace TransformSpace) const {
...
bool ULE_SpringArmComponent::HasAnySockets() const {
	return true;
}

void ULE_SpringArmComponent::QuerySupportedSockets(TArray<FComponentSocketDescription>& OutSockets) const {
	new (OutSockets) FComponentSocketDescription(SocketName, EComponentSocketType::Socket);
}

FVector ULE_SpringArmComponent::GetUnfixedCameraPosition() const {
	return UnfixedCameraPosition;
}

bool ULE_SpringArmComponent::IsCollisionFixApplied() const {
